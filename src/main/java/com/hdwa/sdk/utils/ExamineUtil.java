...
public class ExamineUtil {
    ...
    public static void check_sql(RepositoryBase Repository, SceneProperty sceneProperty, String propertyValueType, String requireSchema,
        ...
        if (propertyValueType != null && propertyValueType.equals("query")) {
            if (sql_json.containsKey("QueryType")) {
                ...
                if (isSubQuery && !QueryType.equals("select")) {
                    errorList.add(
                            new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "SubQuery error QueryType: " + QueryType, null));
                }
                if (isSubQuery || QueryType.equals("select")) {
                    String[] keys = {"QueryType", "Target", "Criteria", "UniqueReturnColumn", "ReturnColumns", "Aggregation", "GroupBy", "OrderBy",
                            "Limit"};
                    ExamineAssistUtil.useless(Repository, sceneProperty, sql_json, keys, errorList);
                    ...
                    if (requireSchema.equals("JSONArray")) {
                        if (requireSingleValueSet) {
                            ...
                            if (sql_json.get("ReturnColumns") != null) {
                                errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "ReturnColumns useless",
                                        sql_json.toString()));
                            }
                            ...
                            if (sql_json.get("UniqueReturnColumn") != null) {
                                errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "UniqueReturnColumn useless",
                                        sql_json.toString()));
                            }
                        } else {
                            if (sql_json.get("UniqueReturnColumn") != null) {
                        ...
                        if (is_SingleAggregation) {
                            errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "Set can't SingleAggregation",
                                    sql_json.toString()));
                        }
                    ...
                    ExamineAssistUtil.check_UniqueReturnColumn(Repository, sceneProperty, sql_json, errorList, check_UniqueReturnColumn);
                    ExamineAssistUtil.check_ReturnColumns(Repository, sceneProperty, sql_json, errorList, check_ReturnColumns);
                    ExamineAssistUtil.check_Aggregation(Repository, sceneProperty, sql_json, errorList, check_SingleAggregation,
                            check_MultiAggregation);
                    ExamineAssistUtil.check_GroupBy(Repository, sceneProperty, sql_json, errorList, check_GroupBy);
                    ExamineAssistUtil.check_OrderBy(Repository, sceneProperty, sql_json, errorList, check_OrderBy);
                    ExamineAssistUtil.check_Limit(Repository, sceneProperty, sql_json, errorList, check_Limit);
                    {
                        Object Target = sql_json.get("Target");
                        checkSet(Repository, sceneProperty, Target, false, errorList);
                    }
                    ...
                    for (String jsonKey : sql_json.keySet()) {
                        if (!jsonKey.equals("QueryType") && !jsonKey.equals("Criteria") && !jsonKey.equals("expression")) {
                            errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "useless key in sql: ", jsonKey));
                        }
                    }
                    ...
                    if (sceneProperty != null
                            && (sceneProperty.propertyValueSchema.equals("JSONArray") || sceneProperty.propertyValueSchema.equals("JSONObject"))) {
                        errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty),
                                "quote propertyValueSchema is " + sceneProperty.propertyValueSchema, null));
                    }
                    JSONObject CriteriaObject = (JSONObject) sql_json.get("Criteria");
                    if (CriteriaObject == null) {
                        errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "quote'Criteria is null", null));
                    } else {
                    ...
                    errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "error QueryType: " + QueryType,
                            sql_json.toString()));
                }
                ...
                if (sql_json.get("Criteria") != null && sql_json.get("Criteria") instanceof JSONObject) {
                    JSONObject CriteriaObject = (JSONObject) sql_json.get("Criteria");
                    checkCriteria(Repository, sceneProperty, CriteriaObject, errorList);
                } else {
                    errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty),
                ...
                checkSet(Repository, sceneProperty, sql_json, requireSingleValueSet, errorList);
                ...
                errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "query error: " + sql_json.toString(),
                        sql_json.toString()));
            }
            ...
            if (QueryType != null && (QueryType.equals("trend") || QueryType.equals("curve"))) {
                JSONObject CriteriaObject = (JSONObject) sql_json.get("Criteria");
                if (CriteriaObject == null) {
                    errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "curve'Criteria is null", null));
                } else {
                ...
                errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "QueryType error", QueryType));
            }
            for (String jsonKey : sql_json.keySet()) {
                if (!jsonKey.equals("QueryType") && !jsonKey.equals("Criteria")) {
                    errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "useless key in sql: ", jsonKey));
            ...
            if (refObject != null && refObject instanceof String) {
                ...
                errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "ref error: " + refObject, sql_json.toString()));
            }
        }
    }

    private static void checkCriteria(RepositoryBase Repository, SceneProperty sceneProperty, JSONObject CriteriaObject,
        ...
        if (LogicOperator != null) {
            if (LogicOperator instanceof String) {
                ...
                if (LogicOperatorString.equals("and") || LogicOperatorString.equals("or")) {
                    ...
                    ExamineAssistUtil.useless(Repository, sceneProperty, CriteriaObject, keys, errorList);
                    Object CriteriasObject = CriteriaObject.get("Criterias");
                    if (CriteriasObject == null || !(CriteriasObject instanceof JSONArray)) {
                        errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "Criterias item error:"
                        ...
                        for (int i = 0; i < Criterias.size(); i++) {
                            ...
                            if (CriteriaObjectInner == null || !(CriteriaObjectInner instanceof JSONObject)) {
                                ...
                                checkCriteria(Repository, sceneProperty, CriteriaObjectInnerJSON, errorList);
                            }
                        }
            ...
            for (String itemKey : CriteriaObject.keySet()) {
                ...
                if (itemValue instanceof JSONObject) {
                    ...
                    if (valueInner.get("ref") != null) {
                        checkCriteriaRef(Repository, sceneProperty, valueInner, errorList);
                    } else {
                    ...
                    errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "Criteria item type error: " + "JSONArray",
                            itemValue.toString()));
                }
            }
        }
    ...
    private static void checkCriteriaRef(RepositoryBase Repository, SceneProperty sceneProperty, JSONObject valueInner, List<ExceptionItem> errorList)
            throws Exception {
        Object refObject = valueInner.get("ref");
        if (!(refObject instanceof String)) {
            errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "ref type error:" + refObject.getClass().getName(),
                    valueInner.toString()));
            return;
        }
    ...
    private static void checkSet(RepositoryBase Repository, SceneProperty sceneProperty, Object setDesc, boolean isSingleValueSet,
                                 List<ExceptionItem> errorList) throws Exception {
        ...
        if (descSet.get("Source") != null) {
            ...
            if (Source.equals("ref")) {
                ...
                if (refObject == null || !(refObject instanceof String)) {
                    ...
                    String refString = (String) refObject;
                    checkSetRef(Repository, sceneProperty, refString, isSingleValueSet, false, errorList);
                }
            } else {
            }
            ...
            if (SetOperatorObject == null || !(SetOperatorObject instanceof String)) {
                errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "SetOperator type error:"
                ...
                if (SetOperator.equals("add") || SetOperator.equals("merge") || SetOperator.equals("unite")) {
                    ...
                    ExamineAssistUtil.useless(Repository, sceneProperty, descSet, keys, errorList);
                    Object SetArrayObject = descSet.get("SetArray");
                    if (SetArrayObject == null || !(SetArrayObject instanceof JSONArray)) {
                        errorList.add(new ExceptionItem(PathUtil.getPropertyPath(Repository, sceneProperty), "SetArray type error:"
                        ...
                        for (Object SetArrayItem : SetArray) {
                            checkSet(Repository, sceneProperty, SetArrayItem, isSingleValueSet, errorList);
                        }
                    ...
                    ExamineAssistUtil.useless(Repository, sceneProperty, descSet, keys, errorList);
                    checkSet(Repository, sceneProperty, descSet.get("Set1"), isSingleValueSet, errorList);
                    checkSet(Repository, sceneProperty, descSet.get("Set2"), isSingleValueSet, errorList);
                }
            }
        } else {
            check_sql(Repository, sceneProperty, "query", "JSONArray", !isSingleValueSet, isSingleValueSet, true, descSet, errorList);
        }
    }

    private static void checkSetRef(RepositoryBase Repository, SceneProperty sceneProperty, String refString, boolean isSingleValueSet,
